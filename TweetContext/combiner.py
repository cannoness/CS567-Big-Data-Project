...
for row in sent_array:
    ...
    if last_user == current_user:
       ...
       user_sent_averaged.append([last_user,clinton/(trump+clinton),[s[1] for s in pol_array if s[0]==last_user]])
       clinton = trump = 0
       user_sent_first_only.append([current_user,row[1],[s[1] for s in pol_array if s[0]==current_user]])
       #add to count
       if float(row[1]) == 1:
           clinton+=1
       else:
